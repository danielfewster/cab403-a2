...
bool load_plates(htab_t *d) {
    ...
    for (int i = 0; i < plates_amount; i++) {
        char *plate = (char *)malloc(plate_chars*sizeof(char));
...
void *level_lpr_sensor(void *data) {
    ...
    while (1) {
        ...
        if ((*args->plates_h)->size < 20) {
            char *plate = (char *)&l->lpr_sensor->license_plate;
...
void *exit_lpr_sensor(void *data) {
    ...
    while (1) {
        ...
        if (el != 0) {
            unsigned long int bill = (time(NULL) - (unsigned long int)el->value) * 1000;

            elt_t *bill_node = DictSearch(*args->plates_billing, el->key);
            if (bill_node == 0) {
                DictInsert(*args->plates_billing, el->key, (char *)&bill);
            } else {
